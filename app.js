...
formWeather.addEventListener('submit', (event) => {
    ...
    if (input.value = input.value.replace(/[^A-Za-z]/ig, '')) {
        ...
        const APIKey = '44e2f2eb5aa6ee84ccd62225d281ffda'; // New API key
        const query = `https://api.openweathermap.com/v1/forecast.json?key=${APIKey}&q=${city}&days=1&aqi=no&alerts=no`;
        ...
        // Handle invalid input with a notification
        const template = `
